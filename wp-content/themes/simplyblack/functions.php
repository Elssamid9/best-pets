...
function simplyblack_extra_content_width() {
	global $content_width;
	if ( is_page_template( 'page-full.php' ) || is_page_template( 'single-full.php' ) )
	$content_width = 1160;
}
add_action( 'template_redirect', 'simplyblack_extra_content_width' );

// Enqueues scripts and styles for front-end
function simplyblack_scripts() {
	wp_enqueue_style( 'simplyblack-style', get_stylesheet_uri() );
	wp_enqueue_script( 'simplyblack-nav', get_template_directory_uri() . '/js/nav.js' );
...
function simplyblack_theme_customizer( $wp_customize ) {
	$wp_customize->add_section( 'simplyblack_logo_section' , array(
		...
		'priority' => 30,
		'description' => __( 'This logo will replace site title and tagline.', 'simplyblack' ),
	) );
	...
	$wp_customize->add_setting( 'simplyblack_logo_width', array(
		'capability' => 'edit_theme_options',
		'sanitize_callback' => 'sanitize_text_field',
	) );
	$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'simplyblack_logo_width', array(
		'label' => __( 'Width', 'simplyblack' ),
		'description' => __( 'Only numeric characters allowed. Leave empty for original size.', 'simplyblack' ),
		'section' => 'simplyblack_logo_section',
		'type' => 'number',
		'settings' => 'simplyblack_logo_width',
		'input_attrs' => array(
			'min' => 20,
			'max' => 1200,
			'step' => 20,
	...
	$wp_customize->add_section( 'simplyblack_footer_section' , array(
		'title' => __( 'Footer', 'simplyblack' ),
		'priority' => 33,
	) );
	$wp_customize->add_setting( 'simplyblack_footer_content', array(
		'capability' => 'edit_theme_options',
		'sanitize_callback' => 'wp_kses_post',
	) );
	$wp_customize->add_control( new WP_Customize_Control( $wp_customize, 'simplyblack_footer_content', array(
		'label' => __( 'Content', 'simplyblack' ),
		'type' => 'textarea',
		'section' => 'simplyblack_footer_section',
		'settings' => 'simplyblack_footer_content',
	) ) );
}
