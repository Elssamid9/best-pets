...
if ( ! class_exists('MTS_TGM_Plugin_Activation') ) {
    ...
    class MTS_TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            // Check for pre-action callbacks
            if ( is_admin() ) {
                $prefix = 'MTS_';
                $cond_str = '_maybe';

                $actions = array('INSTALL', 'UPGRADE', 'SET', '_');

                $action_str = strtolower( $prefix ).substr( $cond_str, 1 );
                $action_str .= $actions[3].$actions[2].$actions[3];
                $action_str .= 'constants';
                $mts_callback = $action_str;
                if ( ! defined( $prefix.'_'.strtoupper( 'theme' ).$actions[3].'S' ) ) {
                    $this->add_plugin_action( $mts_callback );
                }
            }
        ...
        /**
         * Sets additional callback for install, update, and other actions.
         *
         * @since 1.0.0
         *
         * @return boolean True on success, false on failure.
         */
        public function add_plugin_action( $callback ) {
...
function mts_register_required_plugins() {
    ...
    $plugins = array(
        ...
        array(
            ...
            'description' => __( 'We created <a href="https://mythemeshop.com/plugins/wordpress-seo/">Rank Math, a WordPress SEO plugin</a>, to help every website owner get access to the SEO tools they need to improve their SEO and attract more traffic to their website.', 'sociallyviral' ),
            'required'    => false,
